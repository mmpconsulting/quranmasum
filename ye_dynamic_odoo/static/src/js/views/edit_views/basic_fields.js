...
"use strict";
    ...
    var ReportProperty = Widget.extend({
        ...
        bindAction: function () {
            const {onChange} = this.props, {color, background} = this.state.data;
            this.$el.find('input').keyup(this.onKeyUp.bind(this));
            this.$el.find('a:not(".fzColor"):not(".bgColor")').click(this.onClickA.bind(this));
            this.$el.find('.fzColor').spectrum({
                color: color || false,
                change: (color) => onChange({name: "color", value: color.toHexString()}),
            });
            this.$el.find('.bgColor').spectrum({
                color: background || false,
                change: (color) => onChange({name: "background", value: color.toHexString()}),
            });
        },
